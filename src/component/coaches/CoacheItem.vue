<template>
  <li>
    <h3>{{ fullName }}</h3>
    <h4>${{ rate }}/hour</h4>
    <div>
        <base-badge v-for="area in areas" :key="area"
            :title="area"
            :type="area"
        >

        </base-badge>
      <!-- <span v-for="area in areas" :key="area">{{ area }}</span> -->
    </div>
    <div class="actions">
        <base-button mode="outline" link :to="coachContactLink">Contact</base-button>
        <base-button link :to="coachDetailLink">View Detai</base-button>
      <!-- <router-link :to="coachContactLink">Contact</router-link>
      <router-link :to="coachDetailLink">View Detai</router-link> -->
    </div>
  </li>
</template>

<script>
import BaseButton from '../ui/BaseButton.vue';
export default {
  components: { BaseButton },
  props: ['id', 'firstName', 'lastName', 'rate', 'areas', 'descript'],
  computed: {
    fullName() {
      return this.firstName + ' ' + this.lastName;
    },
    coachContactLink() {
      return this.$route.path + '/' + this.id + `/contact`; // /coaches/c1/contact
    },
    coachDetailLink() {
      return this.$route.path + '/' + this.id; ///coaches/c1
    },
  },
};
</script>

<style scoped>
li {
  margin: 1rem 0;
  border: 1px solid #424242;
  border-radius: 12px;
  padding: 1rem;
}

h3 {
  font-size: 1.5rem;
}

h3,
h4 {
  margin: 0.5rem 0;
}

div {
  margin: 0.5rem 0;
}

.actions {
  display: flex;
  justify-content: flex-end;
}
</style>